...
public class GIFCompressor {
    ...
    private class Factory implements ThreadFactory {
        ...
        public Thread newThread(@NonNull Runnable runnable) {
            return new Thread(runnable, "#GwThread-"+count.getAndIncrement());
        }
    }
    ...
    public static boolean cancel(String thread_id){       
        Log.d("GOODVI", "Thread Id at Cancel -> "+thread_id); 
    ...
    public Future<Void> compress(@NonNull final GIFOptions options) {
        final GIFListener listenerWrapper = new ListenerWrapper(options.listenerHandler, options.listener);        
        listenerWrapper.onGIFCompressionCreated("#GwThread-"+threadGen.getAndIncrement());        
    ...
    private static class ListenerWrapper implements GIFListener {
        ...
        @Override
        public void onGIFCompressionCreated(final String thread_id) {
            // Don't think there's a safe way to avoid this allocation?
            // Other than creating a pool of runnables.
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mListener.onGIFCompressionCreated(thread_id);
                }
            });
        }
    }
