...
public class Transcoder {
    ...
    private static AtomicInteger threadGen = new AtomicInteger(1);
    private class Factory implements ThreadFactory {
        private AtomicInteger count = new AtomicInteger(1);
        @Override
        public Thread newThread(@NonNull Runnable runnable) {
            Thread t = new Thread(runnable, "#TwThread-"+count.getAndIncrement());
            Log.d("GOODVIBES", t.getName() + " == " + "#TwThread-"+(threadGen.get() - 1));           
            return t;
        }
    }
    ...
    public static boolean cancel(String thread_id){
        int flag = 1;
        Set<Thread> processes = Thread.getAllStackTraces().keySet();
        for(Thread thread : processes){
            if(thread.getName().equals(thread_id)){
                Log.d("GOODVIBE", thread.getName() +" = "+thread_id+" equals()" );
                thread.interrupt();
                flag = 2;
            }
        }
    ...
    public Future<Void> transcode(@NonNull final TranscoderOptions options) {
        final TranscoderListener listenerWrapper = new ListenerWrapper(options.listenerHandler, options.listener);
        listenerWrapper.onTranscodeCreated("#TwThread-"+threadGen.getAndIncrement());
    ...
    private static class ListenerWrapper implements TranscoderListener {
        ...
        @Override
        public void onTranscodeCreated(final String thread_id) {
            // Don't think there's a safe way to avoid this allocation?
            // Other than creating a pool of runnables.
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mListener.onTranscodeCreated(thread_id);
                }
            });
        }
    }
