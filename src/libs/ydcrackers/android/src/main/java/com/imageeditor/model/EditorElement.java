...
public final class EditorElement implements Parcelable {
  ...
  private static final Comparator<EditorElement> Z_ORDER_COMPARATOR = new Comparator<EditorElement>() {
    @Override
    public int compare(EditorElement e1, EditorElement e2) {
      return Integer.compare(e1.zOrder, e2.zOrder);
    }
  };
  ...
  EditorElement findElement(@NonNull final EditorElement toFind, @NonNull Matrix viewMatrix, @NonNull Matrix outInverseModelMatrix) {
    return findElement(viewMatrix, outInverseModelMatrix, new FindElementPredicate() {
      @Override
      public boolean test(EditorElement element, Matrix inverseMatrix) {
        return toFind == element;
      }
    });
  }
  ...
  EditorElement findElementAt(float x, float y, @NonNull Matrix viewModelMatrix, @NonNull Matrix outInverseModelMatrix) {
    ...
    return findElement(viewModelMatrix, outInverseModelMatrix, new FindElementPredicate() {
      @Override
      public boolean test(EditorElement element, Matrix inverseMatrix) {
