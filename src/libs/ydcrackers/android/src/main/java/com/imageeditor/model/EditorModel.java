...
import com.imageeditor.RendererContext;
import com.imageeditor.UndoRedoStackListener;
...
public final class EditorModel implements Parcelable, RendererContext.Ready {
  ...
  private final boolean reactangleEditing;
  public EditorModel() {
    this(false, false, EditorElementHierarchy.create());
  }

  private EditorModel(@NonNull Parcel in) {
    ClassLoader classLoader     = getClass().getClassLoader();
    Byte b = in.readByte();
    this.circleEditing          = b == 1;
    this.reactangleEditing      = b == 2;
  ...
  public EditorModel(boolean circleEditing, boolean reactangleEditing,  @NonNull EditorElementHierarchy editorElementHierarchy) {
    this.circleEditing          = circleEditing;
    this.reactangleEditing      = reactangleEditing;
  ...
  public static EditorModel create() {
    return new EditorModel(false, false, EditorElementHierarchy.create());
  }

  public static EditorModel createForCircleEditing() {
    EditorModel editorModel = new EditorModel(true, false, EditorElementHierarchy.createForCircleEditing());
    editorModel.setCropAspectLock(true);
    return editorModel;
  }

  public static EditorModel createForReactEditing() {
    EditorModel editorModel = new EditorModel(false, true, EditorElementHierarchy.createForReactEditing());
    editorModel.setCropAspectLock(true);
    return editorModel;
  }
  ...
  public void writeToParcel(Parcel dest, int flags) {
    if(circleEditing)dest.writeByte((byte) 1);
    else if(reactangleEditing)dest.writeByte((byte) 2);
  ...
  public void onReady(@NonNull Renderer renderer, @Nullable Matrix cropMatrix, @Nullable Point size) {
    if (cropMatrix != null && size != null && isRendererOfMainImage(renderer)) {
      ...
      if (imageCropMatrix.isIdentity()) {
        ...
        }else if(reactangleEditing){
          Matrix userCropMatrix = editorElementHierarchy.getCropEditorElement().getLocalMatrix();
          userCropMatrix.setScale(2f, size.x / (float) size.y);
        }
        ...
        if (circleEditing || reactangleEditing) {
          startCrop();
        }
      }
    }
