...
import com.imageeditor.CanvasMatrix;
...
final class AnimationMatrix {
    ...
    if (canAnimate) {
      ...
      animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
        ...
        public void onAnimationUpdate(ValueAnimator valueAnimator) {
          animatedFraction = (float) valueAnimator.getAnimatedValue();
          invalidate.run();
        }
      });
      animator.start();
  ...
  private static Interpolator inverse(@NonNull final Interpolator interpolator) {
    return new Interpolator() {
      @Override
      public float getInterpolation(float input) {
        return 1f - interpolator.getInterpolation(input);
      }
    };
  }
}
