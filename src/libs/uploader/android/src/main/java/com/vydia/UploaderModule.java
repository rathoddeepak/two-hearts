...
import com.vydia.RNUploader.events.NotificationActions;
...
public class UploaderModule extends ReactContextBaseJavaModule {
  ...
  public void startUpload(ReadableMap options, final Promise promise) {
    ...
    try {
      ...
      final MultipartUploadRequest request = new MultipartUploadRequest(this.getReactApplicationContext(), customUploadId, url);
      if (requestType.equals("raw")) {
        //final BinaryUploadRequest request = new BinaryUploadRequest(this.getReactApplicationContext(), customUploadId, url)
                //.setFileToUpload(options.getString("path"));
      } else {
        if (!options.hasKey("files")) {
          promise.reject(new IllegalArgumentException("files is required field for multipart type."));
          return;
        }        
        final ReadableArray files = options.getArray("files");
        for (int i = 0; i < files.size(); i++) {          
          final ReadableMap file = files.getMap(i);                      
          request.addFileToUpload(file.getString("path"), file.getString("field"));          
        }
      }      
      ...
      if (notification.getBoolean("enabled")) {
        ...
        if (notification.hasKey("cancelActionTitle")) {
        notificationConfig.getProgress().actions.add(new UploadNotificationAction(
                R.drawable.ic_cancelled,
                notification.getString("cancelActionTitle"),
                NotificationActions.getCancelUploadAction(this.getReactApplicationContext(), Integer.parseInt(customUploadId), customUploadId)));
        }
