...
export default class SwipeItem extends React.PureComponent<Props, States> {
        ...
        let instance: PanResponderInstance = PanResponder.create({
            ...
            onPanResponderMove: (evt: GestureResponderEvent, gestureState: PanResponderGestureState) => {                
                if(gestureState.dx < 0){
                    this.state.panDistance.setValue({ x: gestureState.dx, y: 0 });
                }                
            },           
    ...
    _renderleftButtonsIfNotNull(): JSX.Element {
        ...
        let scale = this.state.panDistance.x.interpolate({
            inputRange: [0, leftButtonTriggerPosition],
            outputRange: [0, 1],
            extrapolate:'clamp'
        });
    ...
    _renderrightButtonsIfNotNull(): JSX.Element {
        ...
        let scale = this.state.panDistance.x.interpolate({
            ...
            outputRange: [1, 1, 0.7, 0, 0],
            extrapolate:'clamp'
        });

        let opacity = this.state.panDistance.x.interpolate({
            inputRange: [-Infinity, rightButtonTriggerPosition, 0, 0.1, Infinity],
    ...
    render() {
        ...
        return (
            <View style={[style,containerStyles.rootContainer,]}>
                {this._renderleftButtonsIfNotNull()}
                {this._renderrightButtonsIfNotNull()}
                <Animated.View style={[containerStyles.swipeContainer, panStyle]} {...this._panResponder.panHandlers}>{this.props.children}</Animated.View>
            </View>
        );
    }
